...
func (handler *Accounts) GetInternalTxsByAddressHash(ctx *fasthttp.RequestCtx) {
	startTime := time.Now()
	...
	if err != nil {
		handler.logger.Errorf("error getting list internal txs by address hash from blockscout: %v", err)
		prometheus.RecordApiExecTime(recordMethod, strconv.Itoa(-1), "GET", time.Since(startTime).Milliseconds())
		httpapi.InternalServerError(ctx)
		return
	}
...
func (handler *Accounts) GetListTokenTransfersByAddressHash(ctx *fasthttp.RequestCtx) {
	startTime := time.Now()
	...
	if err != nil {
		handler.logger.Errorf("error getting list token transfers by address hash from blockscout: %v", err)
		prometheus.RecordApiExecTime(recordMethod, strconv.Itoa(-1), "GET", time.Since(startTime).Milliseconds())
		httpapi.InternalServerError(ctx)
		return
	}
...
func (handler *Accounts) GetListTokenTransfersByContractAddressHash(ctx *fasthttp.RequestCtx) {
	startTime := time.Now()
