...
import (
	...
	validatorbase_view "github.com/AstraProtocol/astra-indexing/appinterface/projection/rdbvalidatorbase/view"
	...
	for _, event := range events {
		if msgSubmitProposal, ok := event.(*event_usecase.MsgSubmitTextProposal); ok {
			...
			if queryTallyErr != nil {
				return fmt.Errorf("error getting proposal tally from Cosmos api: %v", err)
			}
